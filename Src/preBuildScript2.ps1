...
#upgrade nuspec version.
$files = gci $Env:TF_BUILD_SOURCESDIRECTORY -recurse -include *.nuspec
if($files)
{
	Write-Verbose "Will apply $NewVersion to $($files.count) files."
...
#Replace environement config files


$files = gci $Env:TF_BUILD_SOURCESDIRECTORY -Recurse -Include *.config_$environment
if($files)
{
	Write-Verbose "Will replace $files.Count config files to '$environment' environment."
	foreach ($file in $files) {
			
		#Write-Warning $("Rename " + $file.Split('_')[0]  + "with " + $($file.Split('_')[0] + ".default"))
			
		if(-not $Disable)
		{
			$path = split-Path $file.FullName
			$filename = split-Path $file.FullName -leaf
			#rename old config file to ".default" 
			if (Test-Path($file.FullName.Split('_')[0]))
			{
				Rename-Item -Path $file.FullName.Split('_')[0] -newname $($filename + ".default")
			}	
			
			#rename config file.
			Rename-Item -Path $file -Newname $filename.Split('_')[0]
			Write-Warning "Rename config file '$filename' in folder '$path' to $($filename.Split('_')[0])"
		}
	}
}
else
{
	Write-Warning "Found no files with name *.config_$environment"
}
